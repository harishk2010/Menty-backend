...
export default interface IInstructorControllers {
    ...
    updatePassword(data:{ email: string; password: string }): Promise<IInstructor | null>;
    updateProfile(data:{ email: string; username: string ,profilePicUrl:string }): Promise<IInstructor | null>;
    blockInstructor(data:{email:string,isBlocked:string}): Promise<IInstructor | null>;
}
