...
export default interface IStudentControllers {
    ...
    updatePassword(data:{ email: string; password: string }): Promise<IUser | null>;
    updateProfile(data:{ email: string; username: string ,profilePicUrl:string }): Promise<IUser | null>;
    blockStudent(data:{email:string,isBlocked:string}): Promise<IUser | null>;
